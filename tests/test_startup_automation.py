...
def test_consumer_manifest_declares_the_atlas_integration() -> None:
    ...
    assert config.env_overrides["LIGHTRAG_EXTRACT_LLM_MODEL"] == "qwen3.8:latest"
    assert config.env_overrides["LIGHTRAG_KEYWORD_LLM_MODEL"] == "qwen3.8:latest"
    assert config.env_overrides["LIGHTRAG_QUERY_LLM_MODEL"] == "qwen3.8:latest"
    assert config.env_overrides["RAGAS_EVALUATOR_MODEL"] == "qwen3.8:latest"
    assert config.env_overrides["LITELLM_DEFAULT_MODEL"] == "ollama/qwen3.8:latest"
    assert config.env_overrides["LITELLM_VISION_MODEL"] == "ollama/qwen3.8:latest"
    assert config.env_overrides["OLLAMA_USER_MODELS"] == (
        "qwen3.8:latest,nomic-embed-text"
    )
    assert not config.env_overrides.get("OLLAMA_CUSTOM_MODELS")
