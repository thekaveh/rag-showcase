...
| Atlas Ollama model catalog | `backend_plugins/rag/roles.yaml`, `models.yaml` | `infra/` @ `fe55e838` (`services/ollama/models.yaml`) | `qwen3.6:latest` is a chat model using the `ollama_chat` adapter with model-scoped `request_defaults: {think: false}`; `nomic-embed-text` is the default 768-dimension embedder. The local `models.yaml` compatibility layer remains temporarily and is reviewed in #25. | conformant |
