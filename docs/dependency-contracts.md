...
| Docling document processor (opt-in) | `ingest/ingest.py` | Atlas `--doc-processor-source docling-localhost` / `docling-container-gpu` (disabled by default) | `POST {DOCLING_ENDPOINT}/v1/document/convert` multipart `file` + `{output_format, enable_chunking, chunk_size, chunk_overlap}` → `chunks[].{text, metadata.section_title}`; any failure degrades to naive chunking | conformant |
