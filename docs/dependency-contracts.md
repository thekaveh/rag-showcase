...
| Atlas `_user` compose-overlay discovery | `scripts/setup-overlay.sh`, `compose/rag-overlay.yml` | `infra/` @ `fe55e838` (`bootstrapper/core/docker_manager.py`) | Sorted `services/_user/*/compose.yml` overlays remain supported and merge after the base file with project dir `infra/`; the current symlink target depth (`../../../../compose/rag-overlay.yml`) matches. Migration to Atlas's consumer manifest is tracked in #18. | conformant |
...
| Atlas external env overlay + active `.env` | `config/atlas.env.user`, `scripts/start-all.sh`, `compare/run_matrix.py`, `tests/conftest.py` | `infra/` @ `fe55e838` (`ATLAS_ENV_USER_FILE`, `.env.example`) | Parent-owned defaults provide `PROJECT_NAME`, branding, LightRAG role models/concurrency/timeout, and `OLLAMA_CUSTOM_MODELS`. Atlas merges them into its active `.env`; host tools then read generated ports and secrets from that active file. An alternate absolute `ATLAS_ENV_USER_FILE` is the supported customization path. | conformant |
