...
|---|---|---|---|---|
| Atlas bootstrapper `start.py` (click CLI) | `scripts/start-all.sh` | `infra/` @ `fe55e838` | `gen-ai-rag` includes n8n natively. The wrapper passes `--lightrag-source container`, `--tei-reranker-source container-cpu`, hardware-neutral `--doc-processor-source disabled`, and temporary `--minio-source container` for Atlas #503. Atlas also exposes `--no-tui --detach` / `--no-follow`; wrapper adoption is tracked separately. | conformant with documented #503 workaround |
