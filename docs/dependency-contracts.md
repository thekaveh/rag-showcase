...
| Atlas backend plugin seam | `backend_plugins/rag/__init__.py`, `backend_plugins/requirements.txt`, `compose/rag-overlay.yml` | `infra/` @ `fe55e838` (`services/backend/app/app/plugin_seam.py`, `configure-backend.sh`) | Discovers immediate subdirs of `BACKEND_PLUGINS_DIR` (default `/app/plugins`; multiple roots supported) containing `__init__.py` that exposes a module-level `router`; installs declared plugin dependencies; the overlay's `../backend_plugins:/app/plugins:ro` mount + `BACKEND_PLUGINS_DIR` env match | conformant (route set pinned by `tests/../test_router.py`) |
